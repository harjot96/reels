...
const STEPS = ["Basic Info", "Visual Style", "Publishing"];
    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        ...
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${
              ...
              : step === s ? "bg-primary/20 text-primary border border-primary"
              : "bg-secondary text-muted-foreground"
            }`}>
              {step > s ? <Check className="h-4 w-4" /> : s}
            </div>
            {s < 3 && <div className={`h-0.5 w-12 transition-colors ${step > s ? "bg-primary" : "bg-secondary"}`} />}
          </div>
        ))}
        <span className="ml-2 text-sm text-muted-foreground">{STEPS[step - 1]}</span>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 space-y-5">
        {step === 1 && (
          <>
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">What is this series about?</p>
            </div>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="title">Series Title</Label>
                <Input id="title" placeholder="e.g. Stoic Wisdom Daily" value={form.title}
                  onChange={(e) => update("title", e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="niche">Niche / Topic</Label>
                <Input id="niche" placeholder="e.g. Stoic philosophy quotes and lessons" value={form.niche}
                  onChange={(e) => update("niche", e.target.value)} />
                <p className="text-xs text-muted-foreground">Be specific — this drives the AI script generation</p>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="description">Description (optional)</Label>
                <Textarea id="description" placeholder="Brief description of your channel's purpose"
                  value={form.description} onChange={(e) => update("description", e.target.value)} rows={3} />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex justify-end pt-2">
              <button onClick={() => setStep(2)} disabled={!form.title || !form.niche}
        ...
        {step === 2 && (
          <>
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">Configure how your videos look and sound</p>
            </div>
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {VIDEO_FORMATS.map((f) => (
                    <button key={f.value} type="button" onClick={() => update("videoFormat", f.value)}
                      className={`flex flex-col items-center gap-1 p-3 rounded-lg border-2 transition-colors ${
                        form.videoFormat === f.value ? "border-primary bg-primary/10 text-primary" : "border-border hover:border-primary/50"
                      }`}>
        ...
        {step === 3 && (
          <>
            <div>
              <p className="font-semibold">Publishing Settings</p>
              <p className="text-sm text-muted-foreground mt-0.5">Configure auto-publishing to YouTube</p>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-lg border border-border bg-secondary/20">
                <div>
                  <p className="font-medium text-sm">Auto-publish to YouTube</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Automatically upload generated videos to your YouTube channel
                  </p>
                </div>
                <button type="button" onClick={() => update("autoPublish", !form.autoPublish)}
              ...
              {form.autoPublish && (
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="e.g. 0 9 * * * (daily at 9am)" value={form.publishSchedule}
            ...
            <div className="flex justify-between pt-2">
              ...
              <button onClick={handleSubmit} disabled={loading} className="btn-primary">
                {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating…</> : <><Sparkles className="h-4 w-4" /> Create Series</>}
              </button>
            </div>
          </>
        )}
